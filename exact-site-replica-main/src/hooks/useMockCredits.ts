...
import { mockCreditService } from '@/services/mockCreditService';
import { CreditBalance, CreditPackage } from '@/types/credit';
import { toast } from 'sonner';

export function useMockCredits() {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      const balanceData = await mockCreditService.getCreditBalance();
  ...
  const fetchPackages = useCallback(async () => {
    try {
      const packagesData = await mockCreditService.getCreditPackages();
  ...
  const purchaseCredits = useCallback(async (packageId: number, paymentMethodId: string) => {
    try {
      ...
      const result = await mockCreditService.purchaseCredits(purchaseData);
  ...
  const validateDiscountCode = useCallback(async (code: string) => {
    try {
      const result = await mockCreditService.validateDiscountCode(code);
      return result;
    } catch (err) {
